...
const fromEmail = process.env.RESEND_FROM_EMAIL || "noreply@hvacestimatepro.dev";
          ...
          <tr>
            <td align="center" style="padding: 40px 20px;">
              <table role="presentation" class="email-container" style="max-width: 600px; width: 100%; border-collapse: collapse; background-color: #1A1A1A; border-radius: 12px; overflow: hidden;">
                <!-- Header -->
                <tr>
                  <td class="email-content" style="background: linear-gradient(135deg, #F97316 0%, #EA580C 100%); padding: 40px 30px; text-align: center;">
                ...
                <tr>
                  <td class="email-content" style="background-color: #2A2A2A; padding: 30px; border-top: 1px solid #3A3A3A;">
                    ...
                    <p class="email-muted" style="color: #9CA3AF; font-size: 12px; text-align: center; margin: 0;">
                      © ${currentYear} HVAC Estimate Pro. All rights reserved.<br>
...
export async function sendPasswordResetEmail(
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 40px 0;">
      <a href="${resetUrl}" style="background-color: #F97316; color: #FFFFFF; padding: 16px 32px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px; display: inline-block; transition: background-color 0.2s;">
        Reset Password
      </a>
    </div>
  ...
  const result = await sendEmailWithRetry({
    from: fromEmail,
    to: userEmail,
    subject: "Reset your HVAC Estimate Pro password",
    html,
  });
...
export async function sendWelcomeEmail(
  ...
  const content = `
    <h2 style="color: #FFFFFF; margin-top: 0; font-size: 24px; font-weight: 600;">Welcome to HVAC Estimate Pro!</h2>
    <p class="email-text" style="color: #E5E7EB; font-size: 16px; line-height: 1.6; margin: 20px 0;">
      Hi ${userName || "there"},
    </p>
    <p class="email-text" style="color: #E5E7EB; font-size: 16px; line-height: 1.6; margin: 20px 0;">
      Welcome to HVAC Estimate Pro! We're excited to help you create professional HVAC estimates in 60 seconds.
    </p>
    <div style="background-color: #1A1A1A; border-left: 4px solid #F97316; padding: 20px; margin: 30px 0; border-radius: 4px;">
    ...
    <div style="text-align: center; margin: 40px 0;">
      <a href="${frontendUrl}/dashboard" style="background-color: #F97316; color: #FFFFFF; padding: 16px 32px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px; display: inline-block;">
        Go to Dashboard
      </a>
    </div>
    <p class="email-muted" style="color: #9CA3AF; font-size: 14px; line-height: 1.6; margin: 20px 0;">
      If you have any questions, feel free to reach out to our support team at <a href="mailto:support@hvacestimatepro.dev" style="color: #F97316; text-decoration: none;">support@hvacestimatepro.dev</a>.
    </p>
  `;
  ...
  const result = await sendEmailWithRetry({
    from: fromEmail,
    to: userEmail,
    subject: "Welcome to HVAC Estimate Pro!",
    html,
  });
...
export async function sendSubscriptionConfirmationEmail(
  ...
  const content = `
    ...
    <p class="email-text" style="color: #E5E7EB; font-size: 16px; line-height: 1.6; margin: 20px 0;">
      Thank you for subscribing to HVAC Estimate Pro! Your ${tierName} subscription is now active.
    </p>
    ...
    <p class="email-muted" style="color: #9CA3AF; font-size: 14px; line-height: 1.6; margin: 20px 0;">
      You can manage your subscription anytime from your <a href="${frontendUrl}/settings" style="color: #F97316; text-decoration: none;">account settings</a>.
    </p>
  `;
  ...
  const result = await sendEmailWithRetry({
    from: fromEmail,
    to: userEmail,
    subject: `Welcome to HVAC Estimate Pro ${tierName} Plan!`,
    html,
  });
